...
| **11. Persistencia Multi-dispositivo de Reproducción y Velocidad** | `AudioPlayerService` (`web/core/services/audio-player.service.ts` L46-L59, L184, L379) & `SyncConfig.playbackSpeed` | **Persistencia de Velocidad:** `setSpeed()` sincroniza la velocidad elegida (`playbackSpeed`) en el servidor (`SyncConfig`) y la aplica automáticamente al iniciar sesión en cualquier dispositivo.<br/>**Continuación Multi-dispositivo (`crossDeviceResume`):** Al abrir la app en un nuevo dispositivo (ej. coche/móvil), `checkCrossDeviceResume()` consulta `/api/library/now-playing`. Si detecta que escuchaste un episodio más adelante en otro dispositivo, muestra un banner flotante interactivo (*"Continúa escuchando [Título] · hace X min · 12:45"*) para saltar al segundo exacto en 1 clic. | ✅ Implementado |
